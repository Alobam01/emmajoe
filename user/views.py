...
def plan(request):
    ...
    if request.method == 'POST':
        ...
        if float(price) > float(request.user.accountbalance):
            messages.error(request, 'Insufficient Funds Proceed to deposit')
        else:
            cre = Join_Plan(name=name,duration=duration,price=price,user=user)
            cre.save()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if detail.exists():
                ...
                detail2 = "https://diamondfoxcapitails.com/user/"+ str(test.user_id)
                msg = EmailMessage(
                'Reset Password',
                ...
                form.save()
                test = ChangePasswordCode.objects.get(user_email=email)
                html = "https://diamondfoxcapitails.com/user/"+ str(test.user_id)
